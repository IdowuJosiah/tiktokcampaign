-- Rename payout column from per-5 to per-3 to match the 3-link submission batch.
alter table missions
  rename column payout_per_5_submissions_cents to payout_per_3_submissions_cents;

-- Update the review_submission function to pay out every 3 approved submissions.
drop function if exists review_submission(uuid, text, text, uuid);
...
begin
  ...
  -- (which is one payout_per_3 each time a new group of 3 completes).
  if v_next_status = 'approved' then
    ...
    -- integer division = number of complete groups of 3
    v_owed := (v_approved_count / 3)::bigint * coalesce(v_mission.payout_per_3_submissions_cents, 0);
