    ...
    <AppShell>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {campaigns.length === 0 ? (
          ...
          campaigns.map((c) => (
            <div key={c.id} style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 14, padding: 24, display: "flex", gap: 24 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                  {[c.requiredHashtag, c.requiredSound].filter(Boolean).map((tag) => (
                    <span key={tag} style={{ fontSize: 12, color: "var(--muted)", background: "rgba(0,0,0,0.04)", border: "1px solid var(--line)", borderRadius: 6, padding: "4px 9px" }}>{tag}</span>
                  ))}
                </div>
