  ...
  return (
    <AppShell>
      ...
      <header style={{ position: "relative", minHeight: "100vh", overflow: "hidden" }}>
        ...
        <nav style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 56px", borderBottom: "1px solid var(--line)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            <span style={{ color: "var(--foreground)", fontSize: 14 }}>Home</span>
            <span style={{ color: "var(--muted)", fontSize: 14 }}>How it Works</span>
            <span style={{ color: "var(--muted)", fontSize: 14 }}>For Brands</span>
            <span style={{ color: "var(--muted)", fontSize: 14 }}>Contact</span>
        ...
        <section style={{ position: "relative", display: "flex", gap: 24, justifyContent: "center", flexWrap: "wrap", maxWidth: 1100, margin: "30px auto 0", padding: "0 24px 70px" }}>
          ...
          <div style={{ flex: 1, minWidth: 280, background: "#fff", border: "1px solid var(--line)", borderRadius: 14, padding: 28 }}>
            ...
            <p style={{ color: "var(--muted)", fontSize: 14, lineHeight: 1.55, margin: 0 }}>Withdraw to your bank from ₦2,000 or take a gift card from ₦500. Paystack-powered, no hidden fees.</p>
          </div>
        </section>
      </header>
            ...
            {missions.slice(0, 3).map((m) => (
              <Link key={m.id} href={`/campaigns/${m.id}`} style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 14, padding: 24, display: "flex", gap: 24, alignItems: "center", textDecoration: "none", color: "inherit" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: "var(--muted)", fontSize: 14 }}>{m.brand}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 22, fontWeight: 700, color: "#00d9a3" }}>{m.rewardPool}</div>
                  <div style={{ color: "var(--muted)", fontSize: 12, marginTop: 2 }}>reward pool</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section style={{ maxWidth: 900, margin: "0 auto 80px", padding: "0 24px" }}>
        <div style={{ background: "linear-gradient(135deg,rgba(0,217,163,0.12),rgba(0,217,163,0.03))", border: "1px solid rgba(0,217,163,0.25)", borderRadius: 20, padding: "clamp(36px,6vw,64px)", textAlign: "center" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 16, maxWidth: 520, margin: "0 auto 28px", lineHeight: 1.5 }}>Join creators and brands using transparent submissions, AI scoring, and verified reward flows.</p>
